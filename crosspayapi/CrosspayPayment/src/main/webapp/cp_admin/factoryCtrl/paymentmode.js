angular.module('crossPaymodule')
         ...
         $http.post($localStorage.server_baseurl+'sbank', json).then(function(res){
                                ...
                                console.log("response success"+res.data.status);
                                 if(res.data.status=='200') {
                                	 ...
                                	 showAlert.alertPopup("1","Your transaction was not Successfully Please try again..");
                                	 
                                 }

                     },function(resp){$rootScope.loading = false; console.log(resp);});
	   
	    /*if($localStorage.totamount>1000 && $localStorage.RateData.ccyfrom == 'GBP')showAlert.alertPopup("1","Amount should not cross more than 1000 GBP");
		else{
		   $rootScope.loading = true;
		   var obj = {}
		   ...
		   if($scope.accountdetails=='Bank')obj.commission = $localStorage.RateData.commission;
			...
			if(res.data.status=='200'){
			   ...
			   $('#secur').html($compile(appen)($scope));
				...
				showAlert.alertPopup("1","Please don't refresh the page untill the transaction has completed");
			}else showAlert.alertPopup("1",res.data.message);
		   });
		}*/
    }else{
  ...
  }else showAlert.alertPopup("1","Please select an option to proceed");
