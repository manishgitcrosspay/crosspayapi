    ...
    loadMyDirectives: function($ocLazyLoad) {
     ...
     .then(function() {
      return $ocLazyLoad.load({
       name: 'crossPaymodule',
       serie: true, // load files in series
       files: ['factoryCtrl/forgotPassword.js','js/jqBootstrapValidation.js','js/contact_me.js','js/agency.min.js']
      })
     })
    }
   }
		...
		.when("/profileupdate", {
			...
			controller: "profileUpdateCtrl",
			 resolve: {
			  ...
			  }).when("/transactionunit", {
   templateUrl: "pages/transactionunit.html",
   controller : "transactionunitCtrl",
    resolve: {
    loadMyDirectives: function($ocLazyLoad) {
     return $ocLazyLoad.load([])
     .then(function() {
      return $ocLazyLoad.load({
       name: 'crossPaymodule',
       serie: true, // load files in series
       files: ['factoryCtrl/transactionunit.js','dist/css/sb-admin-2.css','dist/js/sb-admin-2.js']
      })
     })
    }
   }
  }).when("/ratemanager", {
   templateUrl: "pages/ratemanagement.html",
   controller : "ratemanageCtrl",
    resolve: {
    loadMyDirectives: function($ocLazyLoad) {
     return $ocLazyLoad.load([])
     .then(function() {
      return $ocLazyLoad.load({
       name: 'crossPaymodule',
       serie: true, // load files in series
       files: ['factoryCtrl/ratemanage.js','dist/css/sb-admin-2.css','dist/js/sb-admin-2.js']
      })
     })
    }
   }
